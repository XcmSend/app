...
import { setCaretPosition, getCaretPosition, updateCombinedValue, insertPillAtPosition, parseValueToVisualPills  } from './utils';
import { nodeTypeColorMapping } from '../PopupForms/Panel/nodeColorMapping';

const CustomInput = ({ value, onChange, onClick, placeholder, className, pills, setPills,onPillsChange }) => {
...
useEffect(() => {
  // This effect updates the visual representation of the input based on the value prop
  const newValue = parseValueToVisualPills(value, pills, setEditableContent);
  setEditableContent(newValue); // Set the parsed value with visual pills for display
}, [value, pills]);
