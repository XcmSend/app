...
export const handleChange = (path, newValue, replace = false, type, formData) => {
    console.log(`handleChange - ${type} - path:`, path, 'newValue:', newValue, 'replace:', replace);

    let updatedParams = { ...formData.params }; // Clone the existing params

    if (replace) {
        // Clear existing data at the path
        _.unset(updatedParams, path);
        // Set the new value
        _.set(updatedParams, path, newValue);
    } else {
        // Merge or set the new value based on the type
        if (type === 'composite') {
            const currentValue = _.get(updatedParams, path, {});
            const mergedValue = { ...currentValue, ...newValue };
            _.set(updatedParams, path, mergedValue);
        } else {
            _.set(updatedParams, path, newValue);
        }
    }

    console.log('handleChange - updatedParams:', updatedParams);
