...
// Use actionState to fetch PriceInfo:
const fetchPriceInfo = async (currentActionState) => {
  const { action, actionData } = currentActionState;
  ...
  const assetInId = actionData?.assetIn?.assetId;
  const assetOutId = actionData?.assetOut?.assetId;
  const amount = actionData?.assetIn?.amount;
...
useEffect(() => {
  if (selectedNodeId && selectedNodeId.startsWith('action_')) {
      fetchPriceInfo(actionState);
  }
}, [actionState, selectedNodeId]);
...
// When node data changes or when actionData dependencies change:
    useEffect(() => {
      const currentNodeFormData = scenarios[activeScenarioId]?.diagramData?.nodes?.find(node => node.id === nodeId)?.formData;
      const newActionData = convertFormStateToActionType(currentNodeFormData, assetInFormData, assetOutFormData);

      setActionState({
          ...currentNodeFormData,
          actionData: newActionData
      });
    }, [nodeId, activeScenarioId, assetInFormData, assetOutFormData]);
    ...
    useEffect(() => {
      if (activeScenarioId && nodeId) {
          saveNodeFormData(activeScenarioId, nodeId, actionState);
      }
  }, [actionState, nodeId, activeScenarioId]);
